    ...
    <section className="hero-shell relative overflow-hidden text-white">
      ...
      <div className="section-shell relative grid gap-12 py-20 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:py-28">
        <div className="animate-fade-up">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-white/85 backdrop-blur">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
          {stats.map((item, index) => (
            <div
              ...
              <p className="mt-2 text-lg font-semibold leading-snug sm:text-xl">{item.value}</p>
            </div>
          ))}
        </div>
      </div>
